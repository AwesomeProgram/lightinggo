// Code generated by "stringer -type ErrCode -linecomment"; DO NOT EDIT.

package lightinggo

import "strconv"

func _() {
	// An "invalid array index" compiler error signifies that the constant values have changed.
	// Re-run the stringer command to generate them again.
	var x [1]struct{}
	_ = x[NotReadyError-10001]
	_ = x[NotFoundError-10002]
	_ = x[ExpiredError-10003]
	_ = x[ServerError-10101]
	_ = x[TooManyRequests-10102]
	_ = x[ParamBindError-10103]
	_ = x[AuthorizationError-10104]
	_ = x[CallHTTPError-10105]
	_ = x[ResubmitError-10106]
	_ = x[ResubmitMsg-10107]
	_ = x[HashIdsDecodeError-10108]
	_ = x[SignatureError-10109]
	_ = x[IllegalUserName-20101]
	_ = x[UserCreateError-20102]
	_ = x[UserUpdateError-20103]
	_ = x[UserSearchError-20104]
	_ = x[ConfigParseError-20201]
	_ = x[ConfigSaveError-20202]
	_ = x[ConfigRedisConnectError-20203]
	_ = x[ConfigMySQLConnectError-20204]
	_ = x[ConfigMySQLInitError-20205]
	_ = x[ConfigGoVersionError-20206]
	_ = x[SearchRedisError-20501]
	_ = x[ClearRedisError-20502]
	_ = x[SearchRedisEmpty-20503]
	_ = x[SearchMySQLError-20504]
	_ = x[MenuCreateError-20601]
	_ = x[MenuUpdateError-20602]
	_ = x[MenuListError-20603]
	_ = x[MenuDeleteError-20604]
	_ = x[MenuDetailError-20605]
}

const (
	_ErrCode_name_0 = "服务未就绪未找到错误已失效错误"
	_ErrCode_name_1 = "Internal Server ErrorToo Many Requests参数信息有误签名信息有误调用第三方HTTP接口失败ResubmitError请勿重复提交ID参数有误SignatureError"
	_ErrCode_name_2 = "非法用户名创建用户失败更新用户失败查询用户失败"
	_ErrCode_name_3 = "配置文件解析失败配置文件写入失败Redis连接失败MySQL连接失败MySQL初始化数据失败GoVersion不满足要求"
	_ErrCode_name_4 = "查询RedisKey失败清空RedisKey失败查询的RedisKey不存在查询mysql失败"
	_ErrCode_name_5 = "创建菜单失败更新菜单失败删除菜单失败获取菜单列表页失败获取菜单详情失败"
)

var (
	_ErrCode_index_0 = [...]uint8{0, 15, 30, 45}
	_ErrCode_index_1 = [...]uint8{0, 21, 38, 56, 74, 105, 118, 136, 150, 164}
	_ErrCode_index_2 = [...]uint8{0, 15, 33, 51, 69}
	_ErrCode_index_3 = [...]uint8{0, 24, 48, 65, 82, 108, 132}
	_ErrCode_index_4 = [...]uint8{0, 20, 40, 66, 83}
	_ErrCode_index_5 = [...]uint8{0, 18, 36, 54, 81, 105}
)

func (i ErrCode) String() string {
	switch {
	case 10001 <= i && i <= 10003:
		i -= 10001
		return _ErrCode_name_0[_ErrCode_index_0[i]:_ErrCode_index_0[i+1]]
	case 10101 <= i && i <= 10109:
		i -= 10101
		return _ErrCode_name_1[_ErrCode_index_1[i]:_ErrCode_index_1[i+1]]
	case 20101 <= i && i <= 20104:
		i -= 20101
		return _ErrCode_name_2[_ErrCode_index_2[i]:_ErrCode_index_2[i+1]]
	case 20201 <= i && i <= 20206:
		i -= 20201
		return _ErrCode_name_3[_ErrCode_index_3[i]:_ErrCode_index_3[i+1]]
	case 20501 <= i && i <= 20504:
		i -= 20501
		return _ErrCode_name_4[_ErrCode_index_4[i]:_ErrCode_index_4[i+1]]
	case 20601 <= i && i <= 20605:
		i -= 20601
		return _ErrCode_name_5[_ErrCode_index_5[i]:_ErrCode_index_5[i+1]]
	default:
		return "ErrCode(" + strconv.FormatInt(int64(i), 10) + ")"
	}
}
